...
("t"  (let* ((query (if (use-region-p)
              ...
              ;; Map over targets and collect individual command strings into a list
              (commands (mapcar (lambda (target)
                                  (let ((prefix (nth 1 target))
                                        (suffix (nth 2 target)))
                                    (concat "\"" prefix (url-hexify-string query) suffix "\"")))
                                search-targets))
              ;; Stitch them all together into one long, dirty compound string
              (compound-cmd (concat "firefox " (mapconcat #'identity commands " "))))
            (evil-window-split)
            ;; Fire the whole monstrous payload at once
            (buyn-shell-start compound-cmd)

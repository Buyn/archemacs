...
*** p
evil-normal-state-map (kbd "M-p M-o")
(kbd "M-p M-n") [[*kde clipbord history][kde clipbord history]]
...
***** [[*org-roam-menu][org-roam-menu]]
[[*org-insert-menu][org-insert-menu]]
[[*brain-org][brain-org]]
[[*org-macro-menu][org-macro-menu]]
[[*org-roam-menu][org-roam-menu]]
...
***** ai
[[*hydra-ai-insert-menu][hydra-ai-insert-menu]]
...
[[*hydra-applancher-menu][hydra-applancher-menu]]

[[*hydra-shell-scripts-menu][hydra-shell-scripts-menu]]

[[*hydra-eshell-menu][hydra-eshell-menu]]

[[*hydra-player-menu][hydra-player-menu]]

[[*hydra-commands-menu][hydra-commands-menu]]
...
*** [[*INI/org-cfg.el][org-roam]]
[[*INI/ORG/org-roam-cfg.el][INI/ORG/org-roam-cfg.el]]
...
[[*hydra-reader-menu][hydra-reader-menu]]
[[*INI/lit-cfg/read-aloud-cfg.el][INI/lit-cfg/read-aloud-cfg.el]]

*** evil
[[*EVIL-MOD CUSTOMIZATION][EVIL-MOD CUSTOMIZATION]]
...
:END:
*** from
(find-file-other-window "~/.archemacs/init.el")
...
#+end_src
** INI/ORG/org-roam-cfg.el
:PROPERTIES:
:header-args: :tangle INI/ORG/org-roam-cfg.el
:END:
*** описание
(find-file-other-frame "~/.archemacs/INI/ORG/org-roam-cfg.el")

*** used in
(find-file-other-frame "~/.archemacs/INI/org-cfg.el")
...
[[*org-roam-menu][org-roam-menu]]
**** ranger
(ranger "~/Dropbox/orgs/Org-roam/default/")
*** docs
[[file:~/Dropbox/orgs/Org-roam/default/emacs.packages.org_roam.knowledge_base.принципы.org]]
[[file:~/Dropbox/orgs/Org-roam/default/emacs.packages.org-roam.org]]
...
**** ranger
***** keys
Dired-буфер — это можно сделать командой M-x ranger-minimal-toggle или z P
...
The CL util that communicates w/ the engine must wait until the
text was fully pronounced (e.g. not exit immediately), otherwise
...
***** оно не передаёт текс параметром
оно передаёт пайп
***** A Smoke Test
...
It should open 2 tmp windows: 1 log window & 1 w/ a sample text,
then it should start reading automatically. After it finishes you may
...
:END:
*** описание
(find-file-other-frame "~/.archemacs/INI/AI/ai.el")
...
*** used in
[[file:init.el::'(gptel-model 'gemini-flash-lite-latest)]]
;; (find-file "~/.archemacs/INI/init.el")
*** file
(load (expand-file-name "INI/AI/ai.el" user-emacs-directory)) 
*** src INI/AI/ai.el
...
#+RESULTS:
: hydra-ai-menu/body
***** hydra-ai-insert-menu
instead of the image, show prompt message in English, which you would use to get the image.
Instead of an image, show the English prompt that you will use to get the image.
#+begin_src elisp
...
*** docs INI/AI/ai.el
[[id:fe7f1a93-048c-47ba-9546-d1e2fb408f00][emacs Gptel]]

[[id:96341486-cda3-46c8-a3d1-09b59529c377][tech.cyber.ai.google:gemeni]]
...
#+RESULTS:
: #[nil ((projectile-find-references (buffer-substring (region-beginning) (region-end)))) nil nil nil nil]
...
#+end_src
;; --------------------------------------
***** RENPY-MOD CUSTOMIZATION
;; --------------------------------------
;; (find-file "~/.archemacs/INI/dev/renpy-cfg.el")
;; (load "~/.archemacs/INI/dev/renpy-cfg.el")
#+begin_src elisp
...
#+end_src
;; --------------------------------------
***** CANCELED AHK-MODE CUSTOMIZATION
;; --------------------------------------
...
;; --------------------------------------
#+end_src
** INI/dev/renpy-cfg.el
:PROPERTIES:
:header-args: :tangle INI/dev/renpy-cfg.el
:END:
*** описание
...
**** edit
The package comes with the emacs.edit.py script that configures
Ren’py integration with Emacs. Having the RENPY_EDIT_PY
environment variable set to the path of the file would force Ren’py
launcher to use Emacs as a default editor.
#+end_src
...
#+end_src
убрал из конфига -если не отключится то вернуть и устновить состояни на nil
;; *** autoload
...
если есть сомнения в работоспособности Проверь:
...
***** DONE гидра меню быстрого управления
...
***** DONE настроить ивил кнопки в телеге
...
(define-key evil-normal-state-map (kbd "<f8> <S-f8>") #'my/org-save-current-buffer) 
(define-key evil-normal-state-map (kbd "<f8> <f8>") #'my/org-run-remote-or-local-src-block) 
 #+end_src
** package-upgrade
...
* 2025-02-07
** file
(find-file-other-frame "/home/buyn/.archemacs/init.org")
